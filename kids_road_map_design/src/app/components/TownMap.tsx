...
function getDirectionAtDist(d: number): 'right' | 'left' | 'up' | 'down' {
...
function UpDriver({
  cx, cy, direction, isMoving, onClick,
}: {
  cx:number; cy:number;
  ...
  const S = 5; // px per pixel, 8×8 grid = 40×40 SVG
  const bx = cx - 4*S, by = cy - 4*S;

  // Motion trails (behind the driver)
  const trails = isMoving ? [1,2,3].map(i => {
    const op = 0.45 - i*0.13;
    const len = S*(4-i);
    const thin = S/2;
    if (direction==='right') return <rect key={i} x={bx-i*S*2}    y={by+3*S+thin}  width={len} height={thin} fill={UP.sunset} opacity={op} shapeRendering="crispEdges"/>;
    if (direction==='left')  return <rect key={i} x={bx+8*S+i*S}  y={by+3*S+thin}  width={len} height={thin} fill={UP.sunset} opacity={op} shapeRendering="crispEdges"/>;
    if (direction==='up')    return <rect key={i} x={bx+3*S+thin}  y={by+8*S+i*S}  width={thin} height={len} fill={UP.sunset} opacity={op} shapeRendering="crispEdges"/>;
    return                          <rect key={i} x={bx+3*S+thin}  y={by-i*S*2}    width={thin} height={len} fill={UP.sunset} opacity={op} shapeRendering="crispEdges"/>;
  }) : [];

  return (
    <g onClick={onClick} style={{ cursor: 'pointer' }} shapeRendering="crispEdges">
      {/* Ground shadow */}
      ...
      {/* Yellow stepped triangle (pixel-art Up logo triangle) */}
...
export function TownMap({ stops, onSelectStop }: TownMapProps) {
  ...
  const driverDir = getDirectionAtDist(driverDist);

  // ── Travel to a specific stop ──────────────────────────────────────────
  const travelTo = useCallback((stopId: string) => {
    ...
    if (Math.abs(curr - td) < 4) {
      // Already at this stop — open modal immediately
      const stop = stops.find(s => s.id === stopId);
      if (stop) onSelectStop(stop);
      return;
    }
    if (td < curr) {
      // Target is behind — teleport back to start first
      driverDistRef.current = 0;
      setDriverDist(0);
    }
    setTargetDist(td);
  }, [isMoving, stops, onSelectStop]);

  // ── Travel to next stop in sequence ───────────────────────────────────
  const travelToNext = useCallback(() => {
    ...
    travelTo(nextId);
  }, [isMoving, travelTo, stops, onSelectStop]);

  // ── Animation loop ─────────────────────────────────────────────────────
  useEffect(() => {
    if (targetDist === null) return;
    const startDist  = driverDistRef.current;
    const travel     = Math.max(0, targetDist - startDist);
    if (travel < 2) { setTargetDist(null); return; }

    const SPEED    = 500; // SVG units per second
    ...
    const tick = (now: number) => {
      ...
      const d      = startDist + travel * eased;
